...
func PatchHttpSpecie(resolver service.Resolver, logger types.Logger, tracker types.TimeTracker) http.HandlerWithMiddleware {
	proxy := http.NewProxy(
...
func PatchKafkaSpecie(resolver service.Resolver, kafkaInstance *kafka.Kafka, obs *observable.Observable, logger types.Logger, tracker types.TimeTracker, definedTopics kafka.DefinedTopics, paramKey string) http.HandlerWithMiddleware {
	requestHandler := func(response http.Response, request *http.Request) {
		defer tracker(time.Now())
